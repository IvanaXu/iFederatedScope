...
if TEST:
    ...
    ids = [
        ...
        [2, ["cls", "Error rate", 18, get_model1, get_score1, get_predict1]],
        # 3, no edge_attr
        [3, ["cls", "Error rate", 6, get_model1, get_score1, get_predict1]],
        [4, ["cls", "Error rate", 2, get_model1, get_score1, get_predict1]],
        [5, ["cls", "Error rate", 3, get_model1, get_score1, get_predict1]],
        [6, ["cls", "Error rate", 5, get_model1, get_score1, get_predict1]],
        # 7, no edge_attr
        [7, ["cls", "Error rate", 3, get_model1, get_score1, get_predict1]],
        [8, ["cls", "Error rate", 17, get_model1, get_score1, get_predict1]],
...
result, record = [], []
basep = f"{clientp}/base"
for [cid, paras] in ids:
    print(f"\nID {cid}:")
    [task_type, metric, K, model, score, predict] = paras

    train_data = update(pd.read_pickle(f"{pklp}/X-{cid}-train.pkl", compression="zip"), cid, basep)
    valis_data = update(pd.read_pickle(f"{pklp}/X-{cid}-val.pkl", compression="zip"), cid, basep)
    tests_data = update(pd.read_pickle(f"{pklp}/X-{cid}-test.pkl", compression="zip"), cid, basep)
    ...
    with gzip.GzipFile(f"{clientp}/update/{cid}-modelD.mdl", "wb") as f:
